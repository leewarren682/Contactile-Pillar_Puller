...
void loop() {
  ...
  ltoa( millis(),csv_buffer,10); //convert long to charStr
  strcat(dataStr, csv_buffer); //add it to the end
  strcat(dataStr, ", "); //append the delimiter
  
  //dtostrf(floatVal, minimum width, precision, character array);
  dtostrf(mass, 5, 1, csv_buffer);  //5 is minimum width, 1 is precision; float value is copied onto buff
  strcat( dataStr, csv_buffer); //append the converted float
  strcat( dataStr, ", "); //append the delimiter

  dtostrf(platformTravel, 5, 1, csv_buffer);  //5 is minimum width, 1 is precision; float value is copied onto buff
  strcat( dataStr, csv_buffer); //append the converted float
  dataStr[strlen(dataStr)] = '\0'; // terminate correctly
